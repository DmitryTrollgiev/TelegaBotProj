...
def kinokvartal(message):
    ...
    bot.reply_to(message, "Выбранная вами киносеть - https://киноквартал.рф\nДля начала работы выберите город из списка:\n/irkutsk")
...
def check_date(message):
    if message.from_user.id in users_d and users_d[message.from_user.id]["city"] != None and users_d[message.from_user.id]["source"] != None:
